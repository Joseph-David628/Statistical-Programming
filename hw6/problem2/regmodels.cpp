...
void AddRegression(int nMaxRegs, LPRegression regressions,int lenA,int* A,double logmarglikA)
{
  ...
  j=0;  /*num items in list */
  LPRegression r = regressions;
  LPRegression rnext = r->Next;

  /* iterate through list and delete any past ten */
  while(rnext != NULL){
    j+=1;
    r = rnext;
    rnext = r->Next;
  }
  /* if j>nMaxRegs, delete j-nMaxRegs items */
  if(j>nMaxRegs){
    for(i=0;i<(j-nMaxRegs);i++){
      if(j>nMaxRegs){ DeleteLastRegression(regressions); }
    }
  }
  return;
}
